...
void use::Find(const std::filesystem::path& a_settings)
{
	...
	// The one call inside it that can take something off again.
	{
		...
		if (*reinterpret_cast<const std::uint8_t*>(site) != 0xE8) {
			...
			g_equip = reinterpret_cast<decltype(g_equip)>(
				trampoline.write_call<5>(site, &EquipThunk));
			logger::info(
				"use: the equip call at {:#x} comes through here now",
				site - REL::Module::get().base());
		}
	}
